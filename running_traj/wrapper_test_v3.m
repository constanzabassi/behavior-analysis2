for m = 1:25
    [all_frames{m},~,~] =frames_relative2general_pupil_cb(info.mouse_date{m},info.server{m},imaging_st{m})
end
...
SMOOTHING_PARAM = 10;     % in frames how much to smooth by
MIN_PEAK_HEIGHT = .5;    %to find acc peak (in rad2?)
PEAK_PROMINENCE = .1;
turning_threshold = .1;
...
for m = setdiff(all_sessions,weird_sessions); %1:length(imaging_st)
    ...
    non_sst_cells   = setdiff(all_cells, som_cells);
    ...
    % --- Get log-distance to reward (and linear distance if needed) ---
    ...
    for p = 1:length(good_trials)
        ...
        T      = numel(frames);
        % :large_yellow_circle: Use per-frame heading deviation directly (already log-distance aligned)
        hd = heading_deviation{t_idx};  % No need for upsampling
        if numel(hd) ~= T
            warning('Trial %d: mismatch between heading deviation and maze frames (%d vs %d)', ...
                t_idx, numel(hd), T);
            % Optional: skip or pad hd here
        end
        % :large_yellow_circle: Kinematics from heading (for other metrics like turn velocity)
        %load variables
        x_velocity = imaging_array(p).x_velocity(frames);
        view_angle = imaging_array(p).view_angle(frames);
        %get rid of nans for diff in velocity
        view_angle_filled = fillmissing(view_angle, 'linear');

        K  = kinematics_from_heading_local(smooth(view_angle_filled,SMOOTHING_PARAM,'boxcar'), Fs, SMOOTHING_PARAM);
        ...
        if ~isempty(vel_ball{m,1}{p,2})
            ...
            if numel(dist) ~= T
                warning('Trial %d: mismatch between yaw velocity and maze frames (%d vs %d)', ...
                t_idx, numel(dist), T);
            end
        else
        ...
        [~, accel_peaks] = findpeaks(abs(ta), 'MinPeakHeight',MIN_PEAK_HEIGHT,'MinPeakProminence',PEAK_PROMINENCE, ...
...
%                 block = [ETA.nonsst; ETA.som; ETA.heading_dev; ETA.turn_vel; ETA.turn_acc];
        ...
        if ~isempty(tj)
            ...
            if ~isempty(ETA.keep)
                ...
                if abs(ETA.heading_dev(evaluation_timepoint)) > DEV_LARGE
                    %require high deviations at time SPLIT_T_S
                    dev_sign = sign(ETA.heading_dev(evaluation_timepoint));
...
%             valid_peaks = accel_peaks(abs(tv(accel_peaks)) > TV_TH);
            ...
            [~, cand] = findpeaks(abs(ta), 'MinPeakHeight', MIN_PEAK_HEIGHT, ...
                                           'MinPeakProminence', PEAK_PROMINENCE, ...
                                           'MinPeakDistance', round(0.3*Fs));
            cand = cand(abs(tv(cand)) > TV_TH);
...
for c = 1:cols
    ...
    % pull data
    non_lo = group_EVENTS.(low_name).non;    non_hi = group_EVENTS.(high_name).non;
    som_lo = group_EVENTS.(low_name).som;    som_hi = group_EVENTS.(high_name).som;
    hd_lo  = group_EVENTS.(low_name).heading_dev;  hd_hi  = group_EVENTS.(high_name).heading_dev;
    tv_lo  = group_EVENTS.(low_name).tv;     tv_hi  = group_EVENTS.(high_name).tv;
    ta_lo  = group_EVENTS.(low_name).ta;     ta_hi  = group_EVENTS.(high_name).ta;
...
function K = kinematics_from_heading_local(theta, Fs, SMOOTHING_PARAM)
    ...
    turn_vel = dv .* Fs;
%     da = [0, diff(turn_vel)];
    da = [0, diff(smooth(turn_vel,SMOOTHING_PARAM,'boxcar'))];
    K.turn_vel = turn_vel;
    K.turn_acc = da .* Fs;
end
