...
function [acc] = run_classifier_glm_inputs_cluster_specified_cells(current_mouse , save_string_glm,all_celltypes,mdl_param, alignment,info)
...
%     for m = 1:length(imaging_st)
        count = 0;
%         m
        for ce = celltypes_to_do %4th will be all cells together no subsamples!
            count = count+1;
            mdl_param.mouse = m;
            %do current split
%             ex_imaging = imaging_st{1,m};
            ...
            %subsample cells! normally random but now let's get the
            %informative ones
            if ce <4
