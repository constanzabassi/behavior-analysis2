function output = classify_over_time_glm_inputs_cross_time(aligned_data,Y,mdl_param,test_aligned,Y_test)
...
%     t
    ...
    if ismember(0,selected_frames)
       X_test = get_X_nooverlap (test_aligned, mdl_param,0); %frame 0 onset of the event being predicted!
    else %becomes 1 when event onset is an even frame
        X_test = get_X_nooverlap (test_aligned, mdl_param,1); %frame 1 is the frame right after the onset of the event being predicted!
    end
