...
function [acc,shuff_acc,output] = run_classifier_glm_inputs_local_specified_cells(current_mouse , save_string_glm,all_celltypes,mdl_param, alignment,info)
...
%     for m = 1:length(imaging_st)
        count = 0;
%         m
        for ce = 1% %4th will be all cells together no subsamples!
            count = count+1;
            mdl_param.mouse = m;
            %do current split
