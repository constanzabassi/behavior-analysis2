...
function [acc] = run_classifier_glm_inputs_cluster_photostim_updated(current_mouse , save_string_glm,all_celltypes,mdl_param, alignment,info)
...
load(strcat(dir_base, '/updated_condition_array.mat'));
condition_array_trials = updated_condition_array;
% Load imaging data
...
if alignment.active_passive == 1
    load(strcat(base_testing, '/updated_condition_array.mat'));
    condition_array_trials = updated_condition_array;
    test_imaging_spk = make_imaging_from_trials(condition_array_trials, imaging);
else
    load(strcat(dir_base, '/updated_condition_array.mat'));
    condition_array_trials_test = updated_condition_array;
