...
for ce = 1:total_celltypes
    ...
    folds_mean = []; %cellfun(@mean, acc{1,m}(1:10,:,ce));
    folds_mean_shuff = []; %cellfun(@mean, shuff_acc{1,m}(1:10,:,ce));
    for i=0:9
    ...
    if nargin > 5
        ...
        folds_mean_passive = []; %cellfun(@mean, acc{1,m}(1:10,:,ce));
        folds_mean_shuff_passive = []; %cellfun(@mean, shuff_acc{1,m}(1:10,:,ce));
        for i=0:9
        ...
        output_mat2{m,ce}.accuracy = folds_mean_passive; %gives fold means x timepoints
